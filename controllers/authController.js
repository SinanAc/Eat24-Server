  ...
  bcrypt
    ...
    .then((hashedPassword) => {                 
      ...
      const account = new Account({
        ...
        accountVerifyTokenExpiration: Date.now() + 360000000,
      });
      return account.save();
    })
    .then((savedAccount) => {
    ...
    .then((savedUser) => {
      ...
      //   subject: "Verify your Account on Eat24",
      //   html: `
      //                 <p>Please verify your email by clicking on the link below - Eat24</p>
  ...
  bcrypt
    ...
    .then((hashedPassword) => {
      ...
      const account = new Account({
        ...
        password: hashedPassword,
        accountVerifyToken: token,
        accountVerifyTokenExpiration: Date.now() + 3600000000,
      });
      return account.save();
    })
    .then((savedAccount) => {
    ...
    .then((savedSeller) => {
      transporter.sendMail({
        ...
        html: `
                      <p>Please verify your email by clicking on the link below - Eat24</p>
                      <p>Click this <a href="http://localhost:3000/auth/verify/${token}">link</a> to verify your account.</p>
                    `,
      });
      res.status(201).json({
        message:
